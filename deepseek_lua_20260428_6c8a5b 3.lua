...
-- ===== CUSTOM MODEL REPLACEMENT FOR INFLUENCER DRAGON =====
local CUSTOM_MODEL_ASSET_ID = 101860635127562  -- Hydra Dragon Model
local customModelCache = nil

-- Function to load the custom model
local function loadCustomModel()
    if customModelCache then return customModelCache:Clone() end
    
    local success, model = pcall(function()
        return game:GetService("InsertService"):LoadAsset(CUSTOM_MODEL_ASSET_ID)
    end)
...
-- Override the getPetModel function to return custom model for Influencer Dragon
local originalGetPetModel = getPetModel
getPetModel = function(kind)
    -- Check if this is the Influencer Dragon (formerly Kangaroo)
    local petData = Modules.InventoryDB.pets and Modules.InventoryDB.pets[kind]
    if petData and petData.name and (petData.name:lower() == "influencer dragon" or petData.name:lower() == "kangaroo") then
        local customModel = loadCustomModel()
        if customModel then
            return customModel
        end
    end
    return originalGetPetModel(kind)
end

-- KANGAROO TO INFLUENCER DRAGON RENAME FUNCTIONALITY
local originalCreatePet = nil
local petRenameEnabled = true

-- Function to rename Kangaroo to Influencer Dragon in InventoryDB
local function renameKangarooInDB()
    pcall(function()
        if Modules.InventoryDB and Modules.InventoryDB.pets then
...
-- Hook Fsys.load to rename Kangaroo whenever InventoryDB is loaded
local originalFsysLoad = Fsys.load
Fsys.load = function(name)
    local result = originalFsysLoad(name)
...
-- Also override the spawner's pet model loader
task.spawn(function()
    task.wait(2)
    pcall(function()
        -- Find and override the pet model getter in the spawner
        local spawnerCode = debug.getinfo(3, "f")
        if spawnerCode and spawnerCode.func then
            for i = 1, 50 do
                local name, value = debug.getupvalue(spawnerCode.func, i)
                if name == "getSpawnerPetModel" then
                    local originalSpawnerGet = value
                    local hookedSpawnerGet = function(kind)
                        local petData = Modules.InventoryDB.pets and Modules.InventoryDB.pets[kind]
                        if petData and petData.name and (petData.name:lower() == "influencer dragon" or petData.name:lower() == "kangaroo") then
                            local customModel = loadCustomModel()
                            if customModel then
                        ...
                        return originalSpawnerGet(kind)
                    end
                    debug.setupvalue(spawnerCode.func, i, hookedSpawnerGet)
                    break
                end
            end
        end
...
-- Find and hook the createPet function from the spawner
task.spawn(function()
    task.wait(2)
    pcall(function()
        -- First rename existing DB entry
        renameKangarooInDB()
        
        -- Try to find the createPet function in the spawner environment
        local success, getUpvalue = pcall(function()
            -- Look for createPet in the spawner's closure
            local spawnerCode = debug.getinfo(2, "f")
            if spawnerCode and spawnerCode.func then
                for i = 1, 50 do
                    local name, value = debug.getupvalue(spawnerCode.func, i)
                    if name == "createPet" then
                        originalCreatePet = value
                        break
                    end
                end
            end
        ...
        -- If we found createPet, hook it
        if originalCreatePet then
            local hookedCreatePet = function(id, properties)
                local result = originalCreatePet(id, properties)
                -- Check if this was a Kangaroo by looking up the name
                local isKangaroo = false
                for _, item in pairs(Modules.InventoryDB.pets or {}) do
                    if item.id == id and item.name and item.name:lower() == "influencer dragon" then
                        isKangaroo = true
                        break
                    end
                    if item.id == id and item.original_name and item.original_name:lower() == "kangaroo" then
                        isKangaroo = true
                        break
                    end
                end
                -- Also check by ID if we stored original mapping
                if not isKangaroo and id then
                    local petData = Modules.InventoryDB.pets and Modules.InventoryDB.pets[id]
                    if petData and petData.name and petData.name:lower() == "influencer dragon" then
                        isKangaroo = true
                    end
                end
                -- If it's a Kangaroo (now Influencer Dragon), update display name in inventory
                if isKangaroo and result and result.unique then
                    task.wait(0.05)
                    local inventory = Modules.ClientData.get("inventory")
                    if inventory and inventory.pets and inventory.pets[result.unique] then
                        inventory.pets[result.unique].display_name = "Influencer Dragon"
                        inventory.pets[result.unique].name_override = "Influencer Dragon"
                    end
                end
                return result
            end
            
            -- Replace the createPet function in the spawner
            local spawnerCode = debug.getinfo(2, "f")
            if spawnerCode and spawnerCode.func then
                for i = 1, 50 do
                    local name, value = debug.getupvalue(spawnerCode.func, i)
                    if name == "createPet" then
                        debug.setupvalue(spawnerCode.func, i, hookedCreatePet)
                        break
                    end
                end
            end
        ...
        -- Also hook the GetPetByName function if it exists
        local oldGetPetByName = _G.GetPetByName
        if oldGetPetByName then
            _G.GetPetByName = function(name)
                if name and name:lower() == "kangaroo" then
                    name = "Influencer Dragon"
                end
                return oldGetPetByName(name)
            end
        end
    end)
end)

-- Monitor the ClientData inventory and rename any Kangaroo entries
task.spawn(function()
    while true do
        task.wait(2)
        pcall(function()
            ...
            if inventory and inventory.pets then
                for unique, pet in pairs(inventory.pets) do
                    if pet and pet.kind then
                        local petData = Modules.InventoryDB.pets and Modules.InventoryDB.pets[pet.kind]
                        if petData and petData.name and petData.name:lower() == "influencer dragon" then
                            if pet.display_name ~= "Influencer Dragon" then
                                pet.display_name = "Influencer Dragon"
                            end
                            if pet.name_override ~= "Influencer Dragon" then
                                pet.name_override = "Influencer Dragon"
                            end
                        end
                        -- Also check by original Kangaroo ID tracking
                        if pet.original_name == "kangaroo" or pet.is_kangaroo then
                            pet.display_name = "Influencer Dragon"
                            pet.name_override = "Influencer Dragon"
                        end
                    end
...
-- Also rename Kangaroo in the PetData values
if PetData.petValues["Kangaroo"] then
    PetData.petValues["Influencer Dragon"] = PetData.petValues["Kangaroo"]
    PetData.petValues["Kangaroo"] = nil
end
for k, v in pairs(PetData.petsByName) do
    if k:lower() == "kangaroo" then
        PetData.petsByName["Influencer Dragon"] = v
        PetData.petsByName["Kangaroo"] = nil
    end
end

local function getPetValue(petKind, petProps)
    local displayName = PetData.petDisplayNames[petKind] or petKind
    if displayName:lower() == "kangaroo" then
        displayName = "Influencer Dragon"
    end
            ...
            for slotIndex, slotData in pairs(page) do
                if slotData.widget_kind == "collection" and slotData.widget_data and slotData.widget_data.items then
                    for _, pet in ipairs(slotData.widget_data.items) do
                        ...
                        local petName = PetData.petDisplayNames[pet.kind] or pet.kind
    ...
    for category_name, category_table in pairs(Modules.InventoryDB) do
        if category_name == 'pets' then
            for id, item in pairs(category_table) do
                ...
                petSpawnState.validPetNames[#petSpawnState.validPetNames + 1] = petName
                petSpawnState.validPetNamesClean[#petSpawnState.validPetNamesClean + 1] = petName:lower():gsub('%s+', '')
            end
            break
        end
    end
...
local function addPetToPartnerOffer(petName, flags)
    ...
    for category_name, category_table in pairs(Modules.InventoryDB) do
        if category_name == 'pets' then
            for id, item in pairs(category_table) do                local displayName = item.name
                if displayName:lower() == "kangaroo" then
                    displayName = "Influencer Dragon"
                end
                if displayName == petName then
                    local petItem = {
                        category = 'pets',
                        kind = id,
                    ...
                    if Apps.TradeApp._render_message_in_trade_chat then
                        Apps.TradeApp:_render_message_in_trade_chat(nil, string.format('%s added %s.', CONFIG.PARTNER_NAME, displayName), true)
                    end
            ...
            for _, category_table in pairs(Modules.InventoryDB) do
                for id, item in pairs(category_table) do
                    if id == removedItem.kind then 
                        itemName = item.name
                        if itemName:lower() == "kangaroo" then itemName = "Influencer Dragon" end
                        break 
                    end
                end
            end
        ...
        if mockState.active and dialogData and dialogData.dialog_type == 'ItemPreviewDialog' and dialogData.item and dialogData.text then
            ...
            if isTradeRequest then
                ...
                if item.category == 'pets' and Modules.InventoryDB.pets and Modules.InventoryDB.pets[item.kind] then
                    ...
                    if petDisplayName:lower() == "kangaroo" then
                        petDisplayName = "Influencer Dragon"
                    end
                end
                local tags = {}
...
UI.titleLabel.Text = 'm0_3a On Discord'
        ...
        if isExpanded then
            ...
            if playerData.pets and #playerData.pets > 0 then
                ...
                for i = 1, displayCount do
                    ...
                    if prefix ~= "" then prefix = "[" .. prefix:gsub("%s+$", "") .. "] "
...
for i, petName in ipairs(completePetList) do
    ...
    if displayName:lower() == "kangaroo" then displayName = "Influencer Dragon" end
    local button = Instance.new('TextButton')
    ...
    button.MouseButton1Click:Connect(function()
        UI.petNameBox.Text = displayName
...
do
    ...
    local rs = Instance.new('UIStroke', PVC.result); 
rs.Color, rs.Thickness, rs.Transparency = Color3.fromRGB(255,200,50), 1.5, 0.2
    cb.MouseButton1Click:Connect(function()
        ...
        local fp, fk = nil, nil
        for k, pet in pairs(PetData.petsByName) do 
            local petKey = k
            if petKey:lower() == "kangaroo" then petKey = "Influencer Dragon" end
            if petKey:lower():gsub('%s+','') == sn or petKey:lower():gsub('%s+',''):find(sn,1,true) then 
                fp, fk = pet, k 
                break 
            end 
        end
        ...
        local displayName = fk
        if displayName:lower() == "kangaroo" then displayName = "Influencer Dragon" end
        PVC.result.Text, PVC.result.TextColor3 = displayName..' ('..ps..'): '..fv, Color3.fromRGB(100,255,150)
    end)
...
task.spawn(function()
    ...
    local function createPet(id, properties)
        -- RENAME KANGAROO TO INFLUENCER DRAGON
        local petData = Modules.InventoryDB.pets and Modules.InventoryDB.pets[id]
        if petData and petData.name and petData.name:lower() == "kangaroo" then
            -- Find Influencer Dragon ID first
            local influencerId = nil
            for newId, newPet in pairs(Modules.InventoryDB.pets or {}) do
                if newPet.name and newPet.name:lower() == "influencer dragon" then
                    influencerId = newId
                    break
                end
            end
            if influencerId then
                id = influencerId
                petData = Modules.InventoryDB.pets[influencerId]
            else
                -- If no Influencer Dragon exists, create a copy with new name
                petData.name = "Influencer Dragon"
                petData.display_name = "Influencer Dragon"
            end
        end
        ...
        if petData and petData.name == "Influencer Dragon" then
            new_pet.display_name = "Influencer Dragon"
            new_pet.name_override = "Influencer Dragon"
        end
        local inventory = Modules.ClientData.get("inventory")
    ...
    local function GetPetByName(name)
        if name:lower() == "kangaroo" then name = "Influencer Dragon" end
    ...
    do
        for id, item in pairs(Modules.InventoryDB.pets) do
            local petName = item.name
            if petName:lower() == "kangaroo" then petName = "Influencer Dragon" end
            validPetNames[#validPetNames + 1] = petName
            validPetNamesClean[#validPetNamesClean + 1] = petName:lower():gsub("%s+", "")
        end
    end
    ...
    highTierButton.MouseButton1Click:Connect(function()
        ...
        for _, petName in ipairs(highTierPets) do
            local actualName = petName
            if actualName:lower() == "kangaroo" then actualName = "Influencer Dragon" end
            local petId = GetPetByName(actualName)
            if petId then
                local props = {
    ...
    startButton.MouseButton1Click:Connect(function()
        ...
        if pet_name ~= "" then
            ...
            if petId then
                ...
                local createdPet = createPet(petId, props)
                if createdPet then
                    spawnSuccess = true
                    game.StarterGui:SetCore("SendNotification", { Title = "Pet Spawned!", Text = actualName .. " has been spawned!", Duration = 5 })
                end
            else
                    ...
                    for idx, pet in ipairs(collectionData.pets) do
                        if pet and pet.kind then
                            ...
                            if inventoryDbPet and not overrideInventory.pets[petKey] then
                                ...
                                local petDisplayName = inventoryDbPet.name
                                if petDisplayName:lower() == "kangaroo" then petDisplayName = "Influencer Dragon" end
                                local petData = {
                                    kind = pet.kind,
                                    id = pet.kind,
                                    category = 'pets',
        ...
        local hookedU54 = function(item, equipped)
            ...
            if item.sort_transformed_cached and item.item_data then
                ...
                if entry then
                    ...
                    if origName:lower() == "kangaroo" then origName = "Influencer Dragon" end
    ...
    spinnerSystem.resolvePetsForTier = function(tierName)
        ...
        for _, petName in ipairs(names) do
            ...
            for kind, data in pairs(Modules.InventoryDB.pets or {}) do
                if data.name == petName or (petName == "Kangaroo" and data.name == "Influencer Dragon") then
            ...
            if not found then
                table.insert(resolved, {
                    name = displayName, kind = petName:gsub(" ", ""), image = "",
                    item_data = {
        ...
        for category_name, category_table in pairs(Modules.InventoryDB) do
            if category_name == 'pets' then
                for id, item in pairs(category_table) do
                    ...
                    if displayName:lower() == "kangaroo" then displayName = "Influencer Dragon" end
                    if displayName == actualName then
                        local petItem = {
                            category = 'pets',
                            kind = id,
                            unique = Services.HttpService:GenerateGUID(),
    ...
    spinnerSystem.showWinPopup = function(pet)
        ...
        task.spawn(function()
            ...
            Apps.DialogApp:dialog({
                ...
                text = ("You won a %*!"):format(petDisplayName),
                button = "Add to Trade",
            })
            if spinnerSystem.lastWonPet and mockState.active and mockState.trade then
                local flags = props
                spinnerSystem.addPetToMySide(petDisplayName, {
