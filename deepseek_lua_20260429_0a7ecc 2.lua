...
-- WITH INFLUENCER DRAGON INTEGRATION
...
local function resetInfluencerThumbnail()
    ...
    renameKangarooToInfluencer()
    return true, "Thumbnail reset to default"
end
...
-- Add Influencer Dragon to HighTierPets list
local HighTierPets = {
...
-- Insert Influencer Dragon near the top of the list
...
-- Run rename on startup
renameKangarooToInfluencer()

-- Hook FindPetId to recognize Influencer Dragon
local originalFindPetId = nil
...
-- Continue with trade cache and rest of original script...
...
-- Rest of original functions (FetchPetModel, ApplyNeonVisuals, RegisterPetWrapper, etc.)
-- [Keeping all original pet spawn/management functions intact]
...
local function CreateInventoryItem(itemId, category, properties)
    ...
    -- Add Influencer Dragon custom display
...
-- Override router functions (keeping original)
...
local dragonPanel = Instance.new("Frame")
dragonPanel.Size = UDim2.new(0.94, 0, 1, -48)
dragonPanel.Position = UDim2.new(0.03, 0, 0, 46)
dragonPanel.BackgroundTransparency = 1
dragonPanel.Visible = false
dragonPanel.Parent = mainFrame

local dragonTitle = Instance.new("TextLabel")
dragonTitle.Size = UDim2.new(1, 0, 0, 18)
dragonTitle.Position = UDim2.new(0, 0, 0, 0)
dragonTitle.BackgroundTransparency = 1
dragonTitle.Text = "🐉 Influencer Dragon"
dragonTitle.Font = Enum.Font.GothamBold
dragonTitle.TextSize = 11
dragonTitle.TextColor3 = Color3.fromRGB(255, 180, 220)
dragonTitle.TextXAlignment = Enum.TextXAlignment.Left
dragonTitle.Parent = dragonPanel

local dragonDesc = Instance.new("TextLabel")
dragonDesc.Size = UDim2.new(1, 0, 0, 24)
dragonDesc.Position = UDim2.new(0, 0, 0, 20)
dragonDesc.BackgroundTransparency = 1
dragonDesc.Text = "Customize and spawn the exclusive\nInfluencer Dragon pet!"
dragonDesc.Font = Enum.Font.Gotham
dragonDesc.TextSize = 7
dragonDesc.TextColor3 = Color3.fromRGB(180, 180, 220)
dragonDesc.TextXAlignment = Enum.TextXAlignment.Left
dragonDesc.Parent = dragonPanel

-- Thumbnail preview
local thumbnailPreview = Instance.new("ImageLabel")
thumbnailPreview.Size = UDim2.new(0, 60, 0, 60)
thumbnailPreview.Position = UDim2.new(0.65, 0, 0, 20)
thumbnailPreview.BackgroundColor3 = Color3.fromRGB(32, 36, 58)
thumbnailPreview.Image = getInfluencerThumbnail()
thumbnailPreview.ScaleType = Enum.ScaleType.Fit
thumbnailPreview.Parent = dragonPanel
Instance.new("UICorner", thumbnailPreview).CornerRadius = UDim.new(0, 12)

local thumbnailLabel = Instance.new("TextLabel")
thumbnailLabel.Size = UDim2.new(0, 60, 0, 12)
thumbnailLabel.Position = UDim2.new(0.65, 0, 0, 83)
thumbnailLabel.BackgroundTransparency = 1
thumbnailLabel.Text = "Thumbnail"
thumbnailLabel.Font = Enum.Font.Gotham
thumbnailLabel.TextSize = 6
thumbnailLabel.TextColor3 = Color3.fromRGB(140, 140, 180)
thumbnailLabel.TextXAlignment = Enum.TextXAlignment.Center
thumbnailLabel.Parent = dragonPanel

-- Thumbnail input
local thumbLabel = Instance.new("TextLabel")
thumbLabel.Size = UDim2.new(0.6, 0, 0, 10)
thumbLabel.Position = UDim2.new(0, 0, 0, 50)
thumbLabel.BackgroundTransparency = 1
thumbLabel.Text = "🖼️ Thumbnail Decal ID"
thumbLabel.Font = Enum.Font.Gotham
thumbLabel.TextSize = 8
thumbLabel.TextColor3 = Color3.fromRGB(160, 170, 200)
thumbLabel.TextXAlignment = Enum.TextXAlignment.Left
thumbLabel.Parent = dragonPanel

local thumbInput = Instance.new("TextBox")
thumbInput.Size = UDim2.new(0.6, 0, 0, 22)
thumbInput.Position = UDim2.new(0, 0, 0, 61)
...
thumbInput.Text = tostring(INFLUENCER_THUMBNAIL_ID)
thumbInput.Parent = dragonPanel
Instance.new("UICorner", thumbInput).CornerRadius = UDim.new(0, 8)

local updateThumbButton = Instance.new("TextButton")
updateThumbButton.Size = UDim2.new(0.28, 0, 0, 22)
updateThumbButton.Position = UDim2.new(0.62, 0, 0, 61)
updateThumbButton.Text = "Update"
updateThumbButton.Font = Enum.Font.GothamBold
updateThumbButton.TextSize = 9
updateThumbButton.BackgroundColor3 = Color3.fromRGB(100, 100, 200)
updateThumbButton.TextColor3 = Color3.fromRGB(255, 255, 255)
updateThumbButton.Parent = dragonPanel
Instance.new("UICorner", updateThumbButton).CornerRadius = UDim.new(0, 8)

local resetThumbButton = Instance.new("TextButton")
resetThumbButton.Size = UDim2.new(0.28, 0, 0, 20)
resetThumbButton.Position = UDim2.new(0.62, 0, 0, 86)
resetThumbButton.Text = "Reset"
resetThumbButton.Font = Enum.Font.GothamBold
resetThumbButton.TextSize = 8
resetThumbButton.BackgroundColor3 = Color3.fromRGB(180, 100, 100)
resetThumbButton.TextColor3 = Color3.fromRGB(255, 255, 255)
resetThumbButton.Parent = dragonPanel
Instance.new("UICorner", resetThumbButton).CornerRadius = UDim.new(0, 8)

-- Dragon spawn options
local dragonFlagsLabel = Instance.new("TextLabel")
dragonFlagsLabel.Size = UDim2.new(1, 0, 0, 10)
dragonFlagsLabel.Position = UDim2.new(0, 0, 0, 110)
dragonFlagsLabel.BackgroundTransparency = 1
dragonFlagsLabel.Text = "✨ Dragon Flags"
dragonFlagsLabel.Font = Enum.Font.Gotham
dragonFlagsLabel.TextSize = 8
dragonFlagsLabel.TextColor3 = Color3.fromRGB(160, 170, 200)
dragonFlagsLabel.TextXAlignment = Enum.TextXAlignment.Left
dragonFlagsLabel.Parent = dragonPanel

local dragonFlagGrid = Instance.new("Frame")
dragonFlagGrid.Size = UDim2.new(1, 0, 0, 24)
dragonFlagGrid.Position = UDim2.new(0, 0, 0, 121)
dragonFlagGrid.BackgroundTransparency = 1
dragonFlagGrid.Parent = dragonPanel

local dragonFlagState = {M = false, N = false, F = true, R = true}
...
for i, flag in ipairs(flagOrder) do
    ...
    flagButton.Position = UDim2.new((i-1) * 0.25, (i > 1) and 3 or 0, 0, 0)
    flagButton.Text = flag
    flagButton.BackgroundColor3 = dragonFlagState[flag] and flagColors[flag] or Color3.fromRGB(40, 44, 66)
    flagButton.Font = Enum.Font.GothamBold
    flagButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    flagButton.TextSize = 12
    flagButton.Parent = dragonFlagGrid
    Instance.new("UICorner", flagButton).CornerRadius = UDim.new(0, 8)
    
    flagButton.MouseButton1Click:Connect(function()
        if flag == "M" and dragonFlagState["N"] then return end
        if flag == "N" and dragonFlagState["M"] then return end
        
        dragonFlagState[flag] = not dragonFlagState[flag]
        
        if dragonFlagState[flag] then
            flagButton.BackgroundColor3 = flagColors[flag]
        else
            flagButton.BackgroundColor3 = Color3.fromRGB(40, 44, 66)
        end
...
local spawnDragonButton = Instance.new("TextButton")
spawnDragonButton.Size = UDim2.new(1, 0, 0, 32)
spawnDragonButton.Position = UDim2.new(0, 0, 1, -36)
spawnDragonButton.Text = "🐉 SPAWN INFLUENCER DRAGON"
spawnDragonButton.Font = Enum.Font.GothamBold
spawnDragonButton.TextSize = 10
spawnDragonButton.BackgroundColor3 = Color3.fromRGB(200, 80, 180)
spawnDragonButton.TextColor3 = Color3.fromRGB(255, 255, 255)
spawnDragonButton.Parent = dragonPanel
Instance.new("UICorner", spawnDragonButton).CornerRadius = UDim.new(0, 10)
...
tabElements.Dragon.button.MouseButton1Click:Connect(function()
    spawnPanel.Visible = false
    dragonPanel.Visible = true
    toolsPanel.Visible = false
    thumbnailPreview.Image = getInfluencerThumbnail()
end)
...
-- Dragon panel interactions
updateThumbButton.MouseButton1Click:Connect(function()
    local decalId = tonumber(thumbInput.Text)
    if decalId and decalId > 0 then
        local success, msg = setInfluencerThumbnail(decalId)
        if success then
            thumbnailPreview.Image = getInfluencerThumbnail()
            updateThumbButton.Text = "✓ UPDATED!"
            task.wait(1)
            updateThumbButton.Text = "Update"
        else
            updateThumbButton.Text = "❌ INVALID"
            task.wait(1)
            updateThumbButton.Text = "Update"
        end
    else
...
spawnDragonButton.MouseButton1Click:Connect(function()
    local petId = getInfluencerDragonId()
    if not petId then
        spawnDragonButton.Text = "❌ NOT FOUND"
        task.wait(1)
        spawnDragonButton.Text = "🐉 SPAWN INFLUENCER DRAGON"
        return
    end
    ...
    local ageMap = {1, 2, 3, 4, 5, 6}
    local options = {
        mega_neon = dragonFlagState["M"],
        neon = dragonFlagState["N"],
        flyable = dragonFlagState["F"],
        rideable = dragonFlagState["R"],
        age = ageMap[currentAge] or 6,
        trick_level = 5,
        ailments_completed = 0
    }
    ...
    local item = CreateInventoryItem(petId, "pets", options)
    if item then
        spawnDragonButton.Text = "✓ DRAGON SPAWNED!"
        task.wait(1)
        spawnDragonButton.Text = "🐉 SPAWN INFLUENCER DRAGON"
    else
        spawnDragonButton.Text = "❌ FAILED"
        task.wait(1)
        spawnDragonButton.Text = "🐉 SPAWN INFLUENCER DRAGON"
    end
end)
